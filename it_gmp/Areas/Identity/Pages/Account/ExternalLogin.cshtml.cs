...
namespace it.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            var email = authenticationResult.Principal.Identity.Name;
            var user = _context.UserModel.Where(x => x.Email.ToLower() == email.ToLower() && x.deleted_at == null).FirstOrDefault();
            if (user == null)
            {
                /// Audittrail
                var audit1 = new AuditTrailsModel();
                audit1.Type = AuditType.LoginFailed.ToString();
                audit1.DateTime = DateTime.Now;
                audit1.description = $"Tài khoản {email} đăng nhập thất bại.";
                _context.Add(audit1);
